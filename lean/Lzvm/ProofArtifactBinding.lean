...
theorem runtime_proof_artifact_binding_validation_agreement_segment_ids_allowed
  ...
  rcases agreement with
    ⟨_, _, _, _, _, _, _, _, _, segmentIdsAllowedAgreement, _, _⟩
  exact segmentIdsAllowedAgreement artifact publicInput proof
...
theorem runtime_proof_artifact_concrete_segment_id_binding_of_agreement_left
    ...
    RuntimeProofArtifactConcreteSegmentIdBinding left := by
  constructor
...
theorem runtime_proof_artifact_concrete_segment_id_binding_of_agreement_right
    ...
    RuntimeProofArtifactConcreteSegmentIdBinding right := by
  constructor
...
theorem runtime_proof_artifact_finalized_accepts_evidence_core_and_sound
    {system : VerifierModel}
    (assumptions : AssumptionBundle system)
    (validation : RuntimeProofArtifactBindingValidation system) :
    forall artifact publicInput proof,
  ...
  have finalizedContract :=
    runtime_proof_artifact_finalized_evidence_core_and_sound
      ...
      proof
      finalized
  exact And.intro checkedContract.left finalizedContract
          ...
          (And.intro auditedContract.right.right.right.left
            (And.intro auditedContract.right.right.right.right.left
              (And.intro auditedContract.right.right.right.right.right.left
                (And.intro
                  auditedContract.right.right.right.right.right.right
                  concreteSegmentIds))))))
