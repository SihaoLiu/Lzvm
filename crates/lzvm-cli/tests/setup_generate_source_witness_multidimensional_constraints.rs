...
fn temp_dir(name: &str) -> PathBuf {
    std::env::temp_dir().join(format!(
        "lzvm-cli-setup-generate-source-witness-multidimensional-constraints-{}-{name}",
        std::process::id()
    ))
}
...
#[test]
fn generate_key_indexes_multidimensional_witness_array_source_constraints() {
    let dir = temp_dir("witness-multidimensional-array-constraint");
