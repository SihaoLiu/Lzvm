...
#[test]
fn generate_key_lowers_airgroup_global_update_calls_to_global_hints() {
    let dir = temp_dir("airgroup-global-update-hints");
    let _ = fs::remove_dir_all(&dir);
    let source_path = dir.join("source").join("main.pil");
    write_file(
        &source_path,
        "public inputs[4];\n\
         ...
         airgroup GroupA {\n\
             UnitA();\n\
             for (int i = 0; i < 2; ++i) {\n\
                 direct_global_update_proves(BUS_ID, [i, inputs[i * 2], inputs[i * 2 + 1]], surname: LABEL);\n\
             }\n\
         }\n\
         col fixed main.left = [5, 1];",
    );
    ...
    let mut schedule_stdout = Vec::new();
    let mut schedule_stderr = Vec::new();
    let schedule_code = run_cli(
        &[
            "prove",
            "schedule",
            dir.to_str().expect("directory path should be utf-8"),
    ...
    assert_eq!(
        ...
        0,
        "stderr={}",
        String::from_utf8_lossy(&schedule_stderr)
    );
    assert!(String::from_utf8(schedule_stdout)
        .expect("schedule stdout should be utf-8")
        .contains("status=ok\n"));
