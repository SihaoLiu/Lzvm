...
const CUDA_BACKEND_SUMMARY_LINE: &str = if cfg!(feature = "cuda") {
    "cuda_backend=enabled\n"
} else {
...
#[test]
fn rejects_prove_plan_without_setup_directory_manifest() {
    let dir = temp_dir("missing-manifest-plan");
    let _ = fs::remove_dir_all(&dir);
    write_setup_directory(&dir);
    let root = dir.to_str().expect("path should be utf-8");
    let output_dir = dir.join("proof-out");
    run_setup_command(&["setup", "generate-key", root]);
    let manifest_path = remove_setup_manifest(&dir);
    ...
    assert_eq!(
        String::from_utf8(stderr).expect("stderr should be utf-8"),
        format!(
            "prove plan failed: setup directory manifest missing at {}\n",
            manifest_path.display()
        )
    );
}
...
fn rejects_prove_inputs_without_setup_directory_manifest() {
    ...
    assert_eq!(
        String::from_utf8(stderr).expect("stderr should be utf-8"),
        format!(
            "prove inputs failed: setup directory manifest missing at {}\n",
            manifest_path.display()
        )
    );
}
...
#[test]
fn rejects_prove_witness_without_setup_directory_manifest() {
    let dir = temp_dir("missing-manifest-witness");
    let _ = fs::remove_dir_all(&dir);
    write_execution_ready_setup_directory(&dir);
    let catalog = read_key_directory_catalog(&dir).expect("catalog should load");
    let setup_hash = key_directory_catalog_digest(&catalog).expect("digest should compute");
    ...
    assert_eq!(
        String::from_utf8(stderr).expect("stderr should be utf-8"),
        format!(
            "prove witness failed: setup directory manifest missing at {}\n",
            manifest_path.display()
        )
    );
}
...
fn runs_prove_witness_commitments_from_trace_bytes() {
    ...
    assert!(!stdout.contains("timing_"));
    assert!(stderr.is_empty());

    let output_dir_with_timings = dir.join("proof-out-timings");
    let mut stdout = Vec::new();
    let mut stderr = Vec::new();
    let code = run_cli(
        &[
    ...
    let expected_prefix = format!(
        "status=ok\npass=full\nunits=4\nfixed_bytes=128\npcs_material_units=4\npcs_material_bytes={material_bytes}\nqueries=4\nmax_extended_domain_bits=2\npartitions=1\npartition_ids=0\nworker=0\ninput_data={}\naggregate=false\nremote_aggregation=false\nfinal_wrap=false\nverify_outputs=true\nsave_outputs=false\nminimal_memory=false\noutput={}\ngpu_preallocate=false\n{CUDA_BACKEND_SUMMARY_LINE}gpu_streams=20\nwitness_thread_pools=4\nstored_witnesses=4\npack_trace=true\nsetup_hash={setup_hash}\nunit_index=0\ninput_bytes=1\ntrace_rows=2\ntrace_columns=2\nstage_count=2\n{}",
        input_data.display(),
        output_dir_with_timings.display(),
        expected_stages
    );
...
fn segmented_guest_pc_trace_timings_report_internal_aggregates() {
    ...
    assert!(
        stdout.contains("timing_guest_trace_descriptor_high32_stats_enabled="),
        "{stdout}"
    );
    assert!(
    ...
    assert!(
        stdout.contains("timing_guest_stage_trace_extract_ms="),
        "{stdout}"
    );
    assert!(
        stdout.contains("timing_guest_stage_leaf_extend_work_ms="),
        "{stdout}"
    );
    assert!(
...
fn builds_segmented_guest_pc_trace_proof_output() {
    ...
    assert!(
        stdout_text.contains("timing_finish_witness_opening_query_unit_count="),
        "{stdout_text}"
    );
    assert!(
        stdout_text.contains("timing_finish_witness_opening_single_query_unit_count="),
        "{stdout_text}"
    );
    assert!(
        stdout_text.contains("timing_finish_witness_opening_max_queries_per_unit="),
        "{stdout_text}"
    );
    assert!(
...
#[test]
fn runs_single_unit_prove_witness_from_selected_trace_bundle_unit() {
    let dir = temp_dir("prove-witness-trace-bundle-selected-unit");
    let _ = fs::remove_dir_all(&dir);
    write_execution_ready_setup_directory(&dir);
    let output_dir = dir.join("proof-out");
    let guest_image = dir.join("guest.elf");
    ...
    let bundle_bytes = encode_trace_bundle(&TraceBundle {
        units: vec![TraceBundleUnit {
            unit_index: 1,
            trace_bytes: sample_trace_bytes(29),
        }],
    })
    .expect("trace bundle should encode");
    write_bytes(&bundle_path, &bundle_bytes);
    ...
    let stdout = String::from_utf8(stdout).expect("stdout should be utf-8");
    assert!(stdout.contains("unit_index=1\n"));
    assert!(stdout.contains("trace_rows=2\n"));
    assert!(stdout.contains("trace_columns=2\n"));
    assert!(output_dir.join("unit-1.witness-segment").exists());
    assert!(!output_dir.join("unit-0.witness-segment").exists());
    fs::remove_dir_all(&dir).expect("fixture directory should be removed");
}
...
#[test]
fn guest_pc_trace_proves_and_verifies_eth_block_input_with_program_image_cache() {
    let dir = temp_dir("guest-pc-trace-eth-block-input-program-cache");
    let _ = fs::remove_dir_all(&dir);
    let output_dir = dir.join("proof-out");
    let guest_image = dir.join("guest.elf");
    let input_data = dir.join("input.bin");
    ...
    let setup_hash = key_directory_catalog_digest(&catalog).expect("digest should compute");
    write_bytes(&guest_image, sample_guest_pc_trace_image());
    write_bytes(&input_data, framed_stdin_chunk(&[7_u8]));
    write_bytes(&program_path, b"packed-program");
    write_bytes(&constraint_digest_path, setup_hash);
    write_bytes(
        &root_path,
    ...
    assert!(stdout_text.contains("program_image_cache_gpu_mode=cuda\n"));
    assert!(stdout_text.contains("eth_block_input_bytes="));
    assert!(stdout_text.contains("timing_total_ms="));
    assert_eq!(
        verify_code,
        0,
        "{}",
...
#[test]
fn rejects_verify_proof_with_seeded_witness_root_forgery() {
    let dir = temp_dir("verify-proof-seeded-witness-root-forgery");
    let _ = fs::remove_dir_all(&dir);
    write_execution_ready_setup_directory(&dir);
    let catalog = read_key_directory_catalog(&dir).expect("catalog should load");
    let setup_hash = key_directory_catalog_digest(&catalog).expect("digest should compute");
...
#[test]
fn verifies_contribution_set_with_external_program_image_and_eth_block_bindings() {
    let dir = temp_dir("verify-contribution-set-bound-inputs");
    let _ = fs::remove_dir_all(&dir);
    let block_input = build_eth_block_input(&sample_block_rlp()).expect("block input should build");
    write_setup_directory_with_public_values(&dir, &eth_block_public_values_metadata(&block_input));
    let catalog = read_key_directory_catalog(&dir).expect("catalog should parse");
...
#[test]
fn verifies_contribution_set_with_allowed_trailing_eth_public_input_binding() {
    let dir = temp_dir("verify-contribution-set-eth-public-input");
    let _ = fs::remove_dir_all(&dir);
    let public_input = sample_public_block_bytes_with_matching_roots();
    let public_block = parse_eth_public_block_prefix(&public_input).expect("block should parse");
    let block_input = build_eth_block_input(&public_block.block_rlp()).expect("input should build");
...
#[test]
fn rejects_contribution_set_when_later_proof_mismatches_eth_block_binding() {
    let dir = temp_dir("verify-contribution-set-later-block-mismatch");
    let _ = fs::remove_dir_all(&dir);
    let block_input = build_eth_block_input(&sample_block_rlp()).expect("block input should build");
    let mismatched_block_input =
        build_eth_block_input(&sample_block_rlp_variant()).expect("block input should build");
    write_setup_directory_with_public_values(&dir, &eth_block_public_values_metadata(&block_input));
    let catalog = read_key_directory_catalog(&dir).expect("catalog should parse");
    let setup_hash = key_directory_catalog_digest(&catalog).expect("catalog digest should compute");
    ...
    let mismatched_block_segment = ProofSegment {
        id: ETH_BLOCK_INPUT_SEGMENT_ID,
    ...
    assert_eq!(
        String::from_utf8(stderr).expect("stderr should be utf-8"),
        "verify contribution-set failed: ETH block input proof segment mismatch\n"
    );
}

#[test]
fn rejects_contribution_set_when_later_proof_mismatches_program_image_cache_binding() {
    let dir = temp_dir("verify-contribution-set-later-cache-mismatch");
    let _ = fs::remove_dir_all(&dir);
    let block_input = build_eth_block_input(&sample_block_rlp()).expect("block input should build");
    write_setup_directory_with_public_values(&dir, &eth_block_public_values_metadata(&block_input));
    let catalog = read_key_directory_catalog(&dir).expect("catalog should parse");
    ...
    let mismatched_cache_segment = ProofSegment {
        id: PROGRAM_IMAGE_CACHE_SEGMENT_ID,
        data: encode_program_image_cache_segment(&mismatched_cache)
            .expect("cache segment should encode"),
    };
    let block_segment = ProofSegment {
        id: ETH_BLOCK_INPUT_SEGMENT_ID,
    ...
    assert_eq!(
        String::from_utf8(stderr).expect("stderr should be utf-8"),
        "verify contribution-set failed: program image cache proof segment mismatch\n"
    );
}
...
#[test]
fn writes_contribution_challenge_with_external_program_image_and_eth_block_bindings() {
    let dir = temp_dir("write-contribution-challenge-bound-inputs");
    let _ = fs::remove_dir_all(&dir);
    let block_input = build_eth_block_input(&sample_block_rlp()).expect("block input should build");
    write_setup_directory_with_public_values(&dir, &eth_block_public_values_metadata(&block_input));
    let catalog = read_key_directory_catalog(&dir).expect("catalog should parse");
...
struct ContributionChallengeWriterBindingFixture {
    dir: PathBuf,
    proof_a_path: PathBuf,
    proof_b_path: PathBuf,
    public_values_path: PathBuf,
...
fn write_contribution_challenge_writer_binding_fixture(
    name: &str,
    mismatch: ContributionChallengeWriterBindingMismatch,
...
#[test]
fn rejects_writing_contribution_challenge_when_later_proof_mismatches_eth_block_binding() {
    let fixture = write_contribution_challenge_writer_binding_fixture(
        "write-contribution-challenge-later-block-mismatch",
        ContributionChallengeWriterBindingMismatch::EthBlock,
    );
    ...
    assert_eq!(
        String::from_utf8(stderr).expect("stderr should be utf-8"),
        "prove contribution challenges write failed: ETH block input proof segment mismatch\n"
    );
}

#[test]
fn rejects_writing_contribution_challenge_when_later_proof_mismatches_program_image_cache_binding()
{
    let fixture = write_contribution_challenge_writer_binding_fixture(
        "write-contribution-challenge-later-cache-mismatch",
        ContributionChallengeWriterBindingMismatch::ProgramImageCache,
    );
