...
fn temp_dir(name: &str) -> PathBuf {
    std::env::temp_dir().join(format!(
        "lzvm-cli-setup-generate-source-global-static-switch-{}-{name}",
        std::process::id()
    ))
}
...
#[test]
fn generate_key_lowers_static_switch_airgroup_annotations_to_global_hints() {
    let dir = temp_dir("airgroup-annotation-static-switch");
    ...
    write_file(
        ...
        "public inputs[2];\n\
         ...
         airgroup GroupA {\n\
             ...
             switch (selected) {\n\
                 case 0:\n\
                     @record {left: inputs[0], literal: 4}\n\
                 case 1:\n\
                     @record {left: inputs[1], literal: 9}\n\
                 default:\n\
                     @record {left: inputs[0], literal: 11}\n\
             }\n\
         }\n\
