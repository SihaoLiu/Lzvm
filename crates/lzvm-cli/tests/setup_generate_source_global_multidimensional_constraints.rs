...
#[test]
fn generate_key_indexes_multidimensional_public_global_constraints() {
    let dir = temp_dir("public-global-constraint");
    let _ = fs::remove_dir_all(&dir);
    let source_path = dir.join("source").join("main.pil");
    write_file(
        &source_path,
        "public flags[2][2];\n\
         flags[1][0] * (1 - flags[1][0]);\n\
         airtemplate UnitA() { }\n\
...
#[test]
fn generate_key_indexes_multidimensional_proof_value_global_constraints() {
    let dir = temp_dir("proof-value-global-constraint");
    let _ = fs::remove_dir_all(&dir);
    let source_path = dir.join("source").join("main.pil");
    write_file(
        &source_path,
