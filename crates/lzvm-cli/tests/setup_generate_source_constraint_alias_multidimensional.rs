...
fn temp_dir(name: &str) -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("temp")
        .join(format!(
            "lzvm-cli-setup-generate-source-constraint-alias-multidimensional-{}-{name}",
            std::process::id()
        ))
}
...
#[test]
fn generate_key_indexes_multidimensional_expression_array_constraint_aliases() {
    let dir = temp_dir("expression-array-constraint-alias");
