...
use lzvm_artifacts::hint_program::SOURCE_UNSUPPORTED_ASSIGNMENT_HINT;
...
#[test]
fn generate_key_records_unsupported_source_constrained_assignments_as_regular_hints() {
    let dir = temp_dir("unsupported-source-constrained-assignment");
    let _ = fs::remove_dir_all(&dir);
    let source_path = dir.join("source").join("main.pil");
    write_file(
        &source_path,
