...
#[test]
fn generate_key_lowers_public_value_top_level_equality_global_constraints() {
    let dir = temp_dir("public-value-top-level-equality");
...
#[test]
fn generate_key_lowers_final_proof_function_global_constraints() {
    let dir = temp_dir("final-proof-function-global-constraint");
    let _ = fs::remove_dir_all(&dir);
    let source_path = dir.join("source").join("main.pil");
    write_file(
        &source_path,
...
#[test]
fn generate_key_ignores_semicolonless_source_directives_before_global_constraints() {
    let dir = temp_dir("semicolonless-directive-before-global-constraint");
    let _ = fs::remove_dir_all(&dir);
    let source_path = dir.join("source").join("main.pil");
...
#[test]
fn generate_key_lowers_source_function_static_if_public_value_boolean_global_constraints() {
    let dir = temp_dir("source-function-static-if-public-value-boolean");
    let _ = fs::remove_dir_all(&dir);
    let source_path = dir.join("source").join("main.pil");
    write_file(
        &source_path,
...
#[test]
fn generate_key_lowers_source_function_static_for_public_value_boolean_global_constraints() {
    let dir = temp_dir("source-function-static-for-public-value-boolean");
    let _ = fs::remove_dir_all(&dir);
    let source_path = dir.join("source").join("main.pil");
    write_file(
        &source_path,
        "function constrain_flag(expr value, const int count) {\n\
             for (int i = 0; i < count; ++i) {\n\
                 value * (1 - value);\n\
             }\n\
         }\n\
         public flag;\n\
         constrain_flag(flag, 2);\n\
         airtemplate UnitA() { }\n\
         airgroup GroupA { UnitA(); }\n\
         col fixed main.left = [5, 1];",
    );
