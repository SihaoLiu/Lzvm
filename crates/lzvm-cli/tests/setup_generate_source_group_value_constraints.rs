...
fn temp_dir(name: &str) -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("temp")
        .join(format!(
            "lzvm-cli-setup-generate-source-group-value-constraints-{}-{name}",
            std::process::id()
        ))
}
...
#[test]
fn generate_key_lowers_source_air_group_value_constraints() {
    let dir = temp_dir("air-group-value-constraint");
    let _ = fs::remove_dir_all(&dir);
    let source_path = dir.join("source").join("main.pil");
    write_file(
        &source_path,
        "airgroupval stage(2) aggregate(sum) group.expected;\n\
         airtemplate UnitA() {\n\
             col witness value;\n\
    ...
    assert_eq!(setup.group_value_map[0].name, "group.expected");
    ...
    let group_values = [Ext3::new(Felt::from_u64(7), Felt::ZERO, Felt::ZERO)];
    ...
    let extension_group_values = [Ext3::new(Felt::from_u64(7), Felt::from_u64(1), Felt::ZERO)];
    let extension_results = evaluate_regular_constraints(
        ...
        RegularConstraintInputs {
            ...
            stage_columns: &stage_columns,
            opening_point_offsets: &setup.opening_points,
            group_values: &extension_group_values,
            ..RegularConstraintInputs::default()
        },
    )
...
#[test]
fn generate_key_lowers_indexed_source_air_group_value_constraints() {
    let dir = temp_dir("indexed-air-group-value");
    let _ = fs::remove_dir_all(&dir);
    let source_path = dir.join("source").join("main.pil");
    write_file(
        &source_path,
    ...
    assert_eq!(setup.group_value_map[0].name, "group.expected");
    assert_eq!(setup.group_value_map[0].stage, 2);
    assert_eq!(setup.group_value_map[0].lengths, [2]);
    let expressions = read_expression_info_binary_file(
        unit.expression_info_binary()
            .expect("expression metadata path should derive"),
    )
    ...
    let stage_columns = [RegularStageColumns::from_host_values(1, 1, &stage_values)];
    let group_values = [
        Ext3::new(Felt::from_u64(11), Felt::ZERO, Felt::ZERO),
        Ext3::new(Felt::from_u64(22), Felt::ZERO, Felt::ZERO),
    ];
    let results = evaluate_regular_constraints(
        &regular.constraints,
        RegularConstraintInputs {
...
#[test]
fn generate_key_lowers_multidimensional_source_air_group_value_constraints() {
    let dir = temp_dir("multidimensional-air-group-value");
    let _ = fs::remove_dir_all(&dir);
    let source_path = dir.join("source").join("main.pil");
    write_file(
        &source_path,
        "airgroupval stage(2) aggregate(sum) group.expected[2][2];\n\
         airtemplate UnitA() {\n\
             col witness value;\n\
             value === group.expected[1][0];\n\
         }\n\
         airgroup GroupA { UnitA(); }\n\
         col witness stage(2) aux.later;\n\
         col fixed rows = [0, 0];",
    ...
    assert_eq!(setup.group_value_map[0].name, "group.expected");
    assert_eq!(setup.group_value_map[0].stage, 2);
    assert_eq!(setup.group_value_map[0].lengths, [2, 2]);
    let expressions = read_expression_info_binary_file(
        unit.expression_info_binary()
            .expect("expression metadata path should derive"),
    )
...
#[test]
fn generate_key_lowers_template_parameter_static_if_air_group_values() {
    let dir = temp_dir("template-param-static-if-air-group-value");
    let _ = fs::remove_dir_all(&dir);
    let source_path = dir.join("source").join("main.pil");
    write_file(
        &source_path,
        "airtemplate UnitA(const int ENABLED = 1) {\n\
             col witness value;\n\
             if (ENABLED) {\n\
                 airgroupval stage(2) aggregate(sum) group.unused;\n\
                 value === 0;\n\
             } else {\n\
                 airgroupval stage(2) aggregate(sum) group.expected;\n\
                 value === group.expected;\n\
