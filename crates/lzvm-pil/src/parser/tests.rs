...
#[test]
fn parses_fixed_file_pragmas_with_trailing_line_comments() {
    let source = source(
...
#[test]
fn skips_comma_separated_const_declarations() {
    let source = source(
        "function f(const int n) {\n\
           const expr left[n], right[n];\n\
           const int LIMIT = 4;\n\
         }",
    );
...
#[test]
fn parses_sequence_initializer_spans_with_trailing_ellipsis() {
    let source = source("col fixed x = [foo(bar), baz[1]]...;");
