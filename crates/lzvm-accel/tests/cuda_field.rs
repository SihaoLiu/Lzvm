...
fn native_host_header_declares_row_range_extension_exports() {
    ...
    for symbol in [
        ...
        "lzvm_cuda_goldilocks_coset_extend_row_major_columns_shifted_rows_device",
        ...
        "lzvm_cuda_goldilocks_coset_extend_row_major_columns_strided_shifted_rows_device",
    ] {
        assert!(
...
fn cuda_expands_selected_main_trace_descriptor_rows_like_full_layout_slice() {
    ...
    let descriptors = (0..(WORDS_PER_DESCRIPTOR * 2))
        ...
        .collect::<Vec<_>>();
    let full = CudaDeviceBuffer::from_main_trace_descriptors_with_layout(
        descriptors.as_slice(),
        WORDS_PER_DESCRIPTOR,
        2,
        5,
    ...
    let actual =
        CudaDeviceBuffer::from_main_trace_descriptor_selected_row_major_u64_slice_with_layout(
            descriptors.as_slice(),
            WORDS_PER_DESCRIPTOR,
            2,
            5,
...
#[test]
#[cfg(feature = "cuda")]
fn cuda_device_buffer_copies_selected_row_major_rows_from_device() {
    let source = CudaDeviceBuffer::from_u64_words(&(0_u64..20).collect::<Vec<_>>())
        .expect("source device buffer should upload");
    let buffer =
        CudaDeviceBuffer::from_device_selected_row_major_u64_rows(&source, 4, 5, &[2, 0, 3])
            .expect("selected device row-major rows should copy");

    let actual = buffer
        .to_u64_words()
        .expect("selected device rows should download");

    assert_eq!(
        actual,
        vec![10, 11, 12, 13, 14, 0, 1, 2, 3, 4, 15, 16, 17, 18, 19]
    );
}
...
#[test]
#[cfg(feature = "cuda")]
fn cuda_device_buffer_gathers_many_row_major_rows_from_devices() {
    let buffers = (0_u64..17)
        .map(|index| {
            let base = index * 10;
            CudaDeviceBuffer::from_u64_words(&[base, base + 1, base + 2, base + 3])
                .expect("source device buffer should upload")
        })
        .collect::<Vec<_>>();
    let sources = buffers
        .iter()
        .enumerate()
        .map(|(index, buffer)| (buffer, 2, index % 2))
        .collect::<Vec<_>>();

    let buffer = CudaDeviceBuffer::from_device_row_major_u64_rows(&sources, 2)
        .expect("device rows should gather");
    let actual = buffer
        .to_u64_words()
        .expect("gathered device rows should download");
    let expected = (0_u64..17)
        .flat_map(|index| {
            let base = index * 10 + (index % 2) * 2;
            [base, base + 1]
        })
        .collect::<Vec<_>>();
...
#[test]
#[cfg(feature = "cuda")]
fn cuda_extends_shifted_row_major_coset_rows_from_device_memory() {
    let input = vec![5, 9, 2, 1, 9, 4, 3, 7, 6, 8, 11, 10];
    let source_bits = 2;
    let target_bits = 4;
    let column_count = 3;
    ...
    let input_buffer =
        CudaDeviceBuffer::from_u64_words(&input).expect("input device buffer should allocate");
    let mut expected = Vec::with_capacity(target_rows.len() * column_count);
    for target_row in target_rows {
        ...
        .expect("single shifted row extension should run");
        expected.extend(
            row_buffer
                .to_u64_words()
                .expect("single shifted row should download"),
        );
    }

    let mut output_buffer =
        CudaDeviceBuffer::new(expected.len() * 8).expect("row output should allocate");
    cuda_goldilocks_coset_extend_row_major_columns_shifted_rows_device(
        &input_buffer,
        &mut output_buffer,
        column_count,
    ...
    .expect("batched shifted row extension should run");
    let actual = output_buffer
        .to_u64_words()
        .expect("batched shifted rows should download");
...
#[test]
#[cfg(feature = "cuda")]
fn cuda_extends_shifted_strided_row_major_coset_rows_from_device_memory() {
    let source_bits = 2;
    let target_bits = 4;
    let source_rows = 1_usize << source_bits;
    let source_row_stride = 6;
    ...
    for row in 0..source_rows {
        ...
        strided.push(90 + row as u64);
        for column in 0..column_count {
            strided.push((row * 10 + column + 1) as u64);
        }
        strided.push(190 + row as u64);
    }
    let input_buffer =
    ...
    for target_row in target_rows {
        ...
        .expect("single strided shifted row extension should run");
        expected.extend(
            row_buffer
                .to_u64_words()
                .expect("single strided shifted row should download"),
        );
    }
    ...
    .expect("batched strided shifted row extension should run");
    let actual = output_buffer
        .to_u64_words()
        .expect("batched strided shifted rows should download");
...
#[test]
#[cfg(feature = "cuda")]
fn cuda_poseidon2_width8_merkle_digest_opening_prefix_batch_device_accepts_single_query() {
    let digests = (0..17_u64)
        .flat_map(|child| (1..=4).map(move |word| child * 10 + word))
        .collect::<Vec<_>>();
    let padded = padded_digest_states(&digests, 8);
    let input_buffer =
        CudaDeviceBuffer::from_u64_words(&digests).expect("digest buffer should allocate");
    let query_indices = [8_usize];
    let folded_level_count = 3;
    let row_prefix_words = folded_level_count * 4;
    let expected_path = cpu_merkle_opening_width8(&padded, query_indices[0]);
    let expected = expected_path.siblings[..row_prefix_words].to_vec();
...
#[test]
#[cfg(feature = "cuda")]
fn cuda_poseidon2_width8_merkle_digest_opening_prefix_batch_device_buffer_matches_host_batch() {
    let digests = (0..17_u64)
        .flat_map(|child| (1..=4).map(move |word| child * 10 + word))
        .collect::<Vec<_>>();
    let input_buffer =
    ...
    let query_indices = [0_usize, 8, 16];
    let folded_level_count = 3;
    let expected = cuda_poseidon2_width8_merkle_digest_opening_prefix_batch_device(
        &input_buffer,
        &query_indices,
        folded_level_count,
    )
    .expect("host batch prefix should run");

    let actual = cuda_poseidon2_width8_merkle_digest_opening_prefix_batch_device_buffer(
        &input_buffer,
        &query_indices,
        folded_level_count,
    )
    .expect("device-buffer batch prefix should run")
    .to_u64_words()
    .expect("device-buffer batch prefix should download");
...
#[test]
#[cfg(feature = "cuda")]
fn cuda_poseidon2_width16_merkle_digest_opening_prefix_batch_device_accepts_single_query() {
    let digests = (0..70_u64)
        .flat_map(|child| (1..=4).map(move |word| child * 10 + word))
        .collect::<Vec<_>>();
    let padded = padded_digest_states(&digests, 16);
    let input_buffer =
        CudaDeviceBuffer::from_u64_words(&digests).expect("digest buffer should allocate");
    let query_indices = [21_usize];
    let folded_level_count = 3;
    let row_prefix_words = folded_level_count * (4 - 1) * 4;
    let expected_path = cpu_merkle_opening_width16(&padded, query_indices[0]);
    let expected = expected_path.siblings[..row_prefix_words].to_vec();
...
#[test]
#[cfg(feature = "cuda")]
fn cuda_poseidon2_width16_merkle_digest_opening_prefix_batch_device_buffer_matches_host_batch() {
    let digests = (0..70_u64)
        .flat_map(|child| (1..=4).map(move |word| child * 10 + word))
        .collect::<Vec<_>>();
    let input_buffer =
    ...
    let query_indices = [0_usize, 21, 69];
    let folded_level_count = 3;
    let expected = cuda_poseidon2_width16_merkle_digest_opening_prefix_batch_device(
        &input_buffer,
        &query_indices,
        folded_level_count,
    )
    ...
    let actual = cuda_poseidon2_width16_merkle_digest_opening_prefix_batch_device_buffer(
        &input_buffer,
        &query_indices,
        folded_level_count,
    )
