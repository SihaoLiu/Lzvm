...
fn counts_eth_block_input_segments() {
    ...
    assert_eq!(
        ...
        vec![encode_eth_block_input_segment(&block_input)
            .expect("segment should encode")
            .len()]
    );
    assert_eq!(
        report.eth_block_input_block_rlp_byte_counts,
        vec![block_input.block_rlp.len()]
    );
    assert_eq!(
    ...
    assert_eq!(
        report.eth_block_input_base_fees_per_gas,
        vec![block_input.base_fee_per_gas]
    );
    assert_eq!(
    ...
    assert_eq!(
        report.eth_block_input_transaction_roots,
        vec![block_input.transactions_root]
    );
    ...
    assert_eq!(report.eth_block_input_receipts_rlp_byte_counts, vec![None]);
