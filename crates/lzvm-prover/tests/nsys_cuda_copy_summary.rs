...
#[test]
fn nsys_cuda_copy_summary_groups_host_registration_by_application_frame() {
    let crate_root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = crate_root.join("../..");
    let script_path = workspace_root.join("scripts/nsys-cuda-copy-summary.py");
    let temp_dir = workspace_root.join("temp");
    std::fs::create_dir_all(&temp_dir).expect("temp directory should be creatable");
    let sqlite_path = temp_dir.join(format!(
        "nsys-copy-summary-host-registration-frame-test-{}.sqlite",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&sqlite_path);
...
""")
conn.executemany("insert into StringIds (id, value) values (?, ?)", [
    (1, "cudaHostRegister_v3020"),
    (2, "cudaHostUnregister_v3020"),
])
conn.executemany("insert into ENUM_CUDA_MEMCPY_OPER (id, label) values (?, ?)", [
    (1, "Host-to-Device"),
])
...
""", [
    (0, 4_000_000, 1, 200),
    (10_000_000, 17_000_000, 1, 201),
    (20_000_000, 23_000_000, 2, 202),
    (30_000_000, 32_000_000, 1, None),
])
conn.executemany("""
insert into OSRT_CALLCHAINS
    (id, symbol, module, kernelMode, thumbCode, unresolved, specialEntry,
...
values (?, ?, ?, 0, 0, 0, 0, 0, 0, ?)
""", [
    (200, "cudaHostRegister", "libcudart.so", 0),
    (200, "prepare_descriptor_upload", "lzvm", 1),
    (201, "cudaHostRegister", "libcudart.so", 0),
    (201, "prepare_descriptor_upload", "lzvm", 1),
    (202, "cudaHostUnregister", "libcudart.so", 0),
    (202, "release_descriptor_upload", "lzvm", 1),
])
conn.commit()
conn.close()
"#;
    ...
    let setup_output = Command::new("python3")
        ...
        .arg(&sqlite_path)
        .output()
        .expect("host registration frame test database should be created");
    assert!(
        setup_output.status.success(),
        "host registration frame test database creation should succeed: stderr={}",
        String::from_utf8_lossy(&setup_output.stderr)
    );
    ...
    let output = Command::new("python3")
        ...
        .arg(&sqlite_path)
        .output()
        .expect("nsys CUDA copy summary should run on host registration frame test database");
    let _ = std::fs::remove_file(&sqlite_path);
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("host_registration_app_frame_hotspots"),
        "host registration app-frame summary should be printed: {stdout}"
    );
    assert!(
        stdout.contains("cudaHostRegister_v3020,2,11.000,7.000,prepare_descriptor_upload@lzvm"),
        "host registration summary should merge same application frame across callchains: {stdout}"
    );
    assert!(
        stdout.contains("cudaHostRegister_v3020,1,2.000,2.000,unavailable"),
        "host registration summary should keep APIs visible when nsys omits callchains: {stdout}"
    );
    assert!(
        stdout.contains("cudaHostUnregister_v3020,1,3.000,3.000,release_descriptor_upload@lzvm"),
        "host registration summary should keep distinct APIs and frames separate: {stdout}"
    );
}

#[test]
fn nsys_cuda_copy_summary_skips_low_level_copy_wrappers_for_h2d_app_frame() {
    let crate_root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = crate_root.join("../..");
    let script_path = workspace_root.join("scripts/nsys-cuda-copy-summary.py");
    let temp_dir = workspace_root.join("temp");
    std::fs::create_dir_all(&temp_dir).expect("temp directory should be creatable");
    let sqlite_path = temp_dir.join(format!(
        "nsys-copy-summary-low-level-frame-test-{}.sqlite",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&sqlite_path);
...
values (?, ?, ?, 0, 0, 0, 0, 0, 0, ?)
""", [
    (300, "cudaMemcpy", "libcudart.so", 0),
    (300, "lzvm_cuda_copy_h2d_bytes", "lzvm", 1),
    (300, "lzvm_accel::cuda_copy_sites::record_h2d_copy_site_timing", "lzvm", 2),
    (300, "lzvm_accel::cuda_buffer::CudaDeviceBuffer::from_u64_words", "lzvm", 3),
    (300, "lzvm_prover::guest_pc_trace_backend::build_guest_pc_trace_stage_source_devices_from_device_material_timing", "lzvm", 4),
    (300, "lzvm_prover::witness_execution::build_preloaded_guest_pc_trace_stage_source_devices", "lzvm", 5),
])
conn.commit()
conn.close()
"#;
    ...
    let setup_output = Command::new("python3")
        ...
        .arg(&sqlite_path)
        .output()
        .expect("low-level frame test database should be created");
    assert!(
        setup_output.status.success(),
        "low-level frame test database creation should succeed: stderr={}",
        String::from_utf8_lossy(&setup_output.stderr)
    );
    ...
    let output = Command::new("python3")
        ...
        .arg(&sqlite_path)
        .output()
        .expect("nsys CUDA copy summary should run on low-level frame test database");
    let _ = std::fs::remove_file(&sqlite_path);
    ...
    assert!(
        stdout.contains(
            "369098752,1,50.000,44.999,50.000,\
lzvm_prover::guest_pc_trace_backend::build_guest_pc_trace_stage_source_devices_from_device_material_timing@lzvm"
        ),
        "bulk H2D app-frame summary should skip low-level copy wrappers: {stdout}"
    );
}

#[test]
fn nsys_cuda_copy_summary_classifies_guest_trace_descriptor_bulk_uploads() {
    let crate_root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = crate_root.join("../..");
    let script_path = workspace_root.join("scripts/nsys-cuda-copy-summary.py");
    let temp_dir = workspace_root.join("temp");
    std::fs::create_dir_all(&temp_dir).expect("temp directory should be creatable");
    let sqlite_path = temp_dir.join(format!(
        "nsys-copy-summary-guest-descriptor-h2d-test-{}.sqlite",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&sqlite_path);
...
""", [
    (300, "cudaMemcpy", "libcudart.so", 0),
    (300, "lzvm_cuda_copy_h2d_bytes", "lzvm", 1),
    (300, "lzvm_accel::cuda_buffer::CudaDeviceBuffer::from_u64_words", "lzvm", 2),
    (300, "lzvm_prover::guest_pc_trace_backend::build_guest_pc_trace_stage_source_devices_from_device_material_timing", "lzvm", 3),
])
conn.commit()
conn.close()
"#;
    ...
    let setup_output = Command::new("python3")
        ...
        .arg(&sqlite_path)
        .output()
        .expect("guest descriptor H2D test database should be created");
    assert!(
        setup_output.status.success(),
        "guest descriptor H2D test database creation should succeed: stderr={}",
        String::from_utf8_lossy(&setup_output.stderr)
    );
    ...
    let output = Command::new("python3")
        ...
        .arg(&sqlite_path)
        .output()
        .expect("nsys CUDA copy summary should run on guest descriptor H2D test database");
    let _ = std::fs::remove_file(&sqlite_path);
    ...
    assert!(
        stdout.contains(
            "h2d_structural_hint,trace_descriptor_residency_pipeline,bytes=369098752 calls=1 host_api_ms=80.000"
        ),
        "transfer triage should classify repeated guest trace descriptor uploads as a structural residency pipeline candidate: {stdout}"
    );
}
...
fn nsys_cuda_copy_summary_skips_native_direct_d2h_wrapper_for_app_frame() {
    ...
    std::fs::create_dir_all(&temp_dir).expect("temp directory should be creatable");
    let sqlite_path = temp_dir.join(format!(
        "nsys-copy-summary-d2h-low-level-frame-test-{}.sqlite",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&sqlite_path);
...
insert into CUPTI_ACTIVITY_KIND_MEMCPY
    (start, end, deviceId, contextId, greenContextId, streamId, correlationId,
     ...
     srcContextId, dstDeviceId, dstContextId, migrationCause, graphNodeId,
     virtualAddress, copyCount)
values (1000, 2000, 0, 0, null, 3, 45, 1, 1152, 1, null, null, null,
        null, null, null, null, null, null, null, 1)
""")
conn.executemany("""
insert into OSRT_CALLCHAINS
...
""", [
    (301, "cudaMemcpy", "libcudart.so", 0),
    (301, "(anonymous namespace)::record_direct_d2h_copy(void*, void const*, unsigned long)", "lzvm", 1),
    (301, "lzvm_cuda_record_direct_copy_d2h_wait", "lzvm", 2),
    (301, "lzvm_prover::witness_commitment::values::copy_extended_row_values_from_device", "lzvm", 3),
    (301, "lzvm_prover::pcs_fri::build::build_pcs_fri_opening_segment", "lzvm", 4),
])
conn.commit()
conn.close()
"#;
    ...
    let setup_output = Command::new("python3")
        ...
        .arg(&sqlite_path)
        .output()
        .expect("D2H low-level frame test database should be created");
    assert!(
        setup_output.status.success(),
        "D2H low-level frame test database creation should succeed: stderr={}",
        String::from_utf8_lossy(&setup_output.stderr)
    );
    ...
    let output = Command::new("python3")
        ...
        .arg(&sqlite_path)
        .output()
        .expect("nsys CUDA copy summary should run on D2H low-level frame test database");
    let _ = std::fs::remove_file(&sqlite_path);
    ...
    assert!(
        stdout.contains(
            "cudaMemcpy,1152,1,12.000,0.001,12.000,301,\
lzvm_prover::witness_commitment::values::copy_extended_row_values_from_device@lzvm,"
        ),
        "D2H app-frame summary should skip native direct-copy wrappers: {stdout}"
    );
}

#[test]
fn nsys_cuda_copy_summary_groups_d2h_wait_by_application_frame() {
    let crate_root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = crate_root.join("../..");
    let script_path = workspace_root.join("scripts/nsys-cuda-copy-summary.py");
    let temp_dir = workspace_root.join("temp");
    std::fs::create_dir_all(&temp_dir).expect("temp directory should be creatable");
    let sqlite_path = temp_dir.join(format!(
        "nsys-copy-summary-d2h-app-frame-test-{}.sqlite",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&sqlite_path);
...
""", [
    (301, "cudaMemcpy", "libcudart.so", 0),
    (301, "(anonymous namespace)::record_direct_d2h_copy(void*, void const*, unsigned long)", "lzvm", 1),
    (301, "lzvm_prover::witness_commitment::values::copy_extended_row_values_from_device", "lzvm", 2),
    (302, "cudaMemcpy", "libcudart.so", 0),
    (302, "(anonymous namespace)::record_direct_d2h_copy(void*, void const*, unsigned long)", "lzvm", 1),
    (302, "lzvm_prover::witness_commitment::values::copy_extended_row_values_from_device", "lzvm", 2),
])
conn.commit()
conn.close()
"#;
    ...
    let setup_output = Command::new("python3")
        ...
        .arg(&sqlite_path)
        .output()
        .expect("D2H app-frame test database should be created");
    assert!(
        setup_output.status.success(),
        "D2H app-frame test database creation should succeed: stderr={}",
        String::from_utf8_lossy(&setup_output.stderr)
    );
    ...
    let output = Command::new("python3")
        ...
        .arg(&sqlite_path)
        .output()
        .expect("nsys CUDA copy summary should run on D2H app-frame test database");
    let _ = std::fs::remove_file(&sqlite_path);
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("d2h_app_frame_hotspots"),
        "D2H app-frame summary should be printed: {stdout}"
    );
    assert!(
        stdout.contains(
            "1152,2,20.000,0.002,12.000,\
lzvm_prover::witness_commitment::values::copy_extended_row_values_from_device@lzvm"
        ),
        "same D2H app frame should be aggregated across callchains: {stdout}"
    );
}
...
fn nsys_cuda_copy_summary_prefers_cuda_callchains_for_cuda_memcpy_frames() {
    ...
    std::fs::create_dir_all(&temp_dir).expect("temp directory should be creatable");
    let sqlite_path = temp_dir.join(format!(
        "nsys-copy-summary-cuda-callchain-test-{}.sqlite",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&sqlite_path);
...
conn.executemany("insert into StringIds (id, value) values (?, ?)", [
    (1, "cudaMemcpy"),
    (2, "libcudart.so"),
    (3, "lzvm_cuda_copy_h2d_bytes"),
    (4, "lzvm"),
    (5, "lzvm_accel::cuda_buffer::CudaDeviceBuffer::from_u64_words"),
    (6, "lzvm_prover::guest_pc_trace_backend::build_guest_pc_trace_stage_source_devices_from_device_material_timing"),
])
conn.executemany("insert into ENUM_CUDA_MEMCPY_OPER (id, label) values (?, ?)", [
    (1, "Host-to-Device"),
])
...
values (?, ?, ?, 0, 0, 0, 0, 0, 0, ?)
""", [
    (400, "lzvm_cuda_memory_info", "lzvm", 0),
])
conn.executemany("""
insert into CUDA_CALLCHAINS
    (id, symbol, module, unresolved, originalIP, stackDepth)
values (?, ?, ?, 0, 0, ?)
""", [
    (400, 1, 2, 0),
    (400, 3, 4, 1),
...
"#;
    ...
    let setup_output = Command::new("python3")
        ...
        .arg(&sqlite_path)
        .output()
        .expect("CUDA callchain test database should be created");
    assert!(
        setup_output.status.success(),
        "CUDA callchain test database creation should succeed: stderr={}",
        String::from_utf8_lossy(&setup_output.stderr)
    );
    ...
    let output = Command::new("python3")
        ...
        .arg(&sqlite_path)
        .output()
        .expect("nsys CUDA copy summary should run on CUDA callchain test database");
    let _ = std::fs::remove_file(&sqlite_path);
...
lzvm_prover::guest_pc_trace_backend::build_guest_pc_trace_stage_source_devices_from_device_material_timing@lzvm"
        ...
        "CUDA memcpy app-frame summary should prefer CUDA_CALLCHAINS over OSRT callchains: {stdout}"
    );
    assert!(
        !stdout.contains("369098752,1,50.000,44.999,50.000,lzvm_cuda_memory_info@lzvm"),
        "OSRT memory-info frames should not hide CUDA memcpy callchains: {stdout}"
    );
}
