...
#[test]
fn improve_log_check_accepts_legacy_probe_timing_labels() {
    let crate_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = crate_root
        .parent()
        .and_then(std::path::Path::parent)
    ...
    let script_path = workspace_root.join("scripts/append-improve-log.py");
    let log_path = workspace_root.join(format!(
        "temp/improve-log-legacy-probes-{}.csv",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&log_path);
    std::fs::write(
        ...
        concat!(
            "timestamp,commit,small_proof_time_s,large_proof_time_s,summary\n",
            "\"2026-06-19T05:55:00-0700\",\"testcase\",\"probe_5m=1.44\",",
            "\"probe_50m=11.92\",\"Legacy probe labels\"\n",
            "\"2026-06-19T06:55:00-0700\",\"testcase\",\"\",",
            "\"copy_gate_avg=42.166 samples=42.136;42.313;42.049\",\"Legacy named average\"\n",
            "\"2026-06-19T07:55:00-0700\",\"testcase\",\"\",",
            "\"regressed single=43.34\",\"Legacy single run\"\n",
            "\"2026-06-19T08:55:00-0700\",\"testcase\",\"\",",
            "\"timeout>180\",\"Legacy timeout marker\"\n",
        ),
    )
    .expect("temporary improve log should write");
    ...
    assert!(
        output.status.success(),
        "improve-log check should accept legacy timing labels: stderr={}",
        String::from_utf8_lossy(&output.stderr)
    );
}
...
#[cfg(unix)]
#[test]
fn improve_log_check_rejects_symlinked_log_path() {
    use std::os::unix::fs::symlink;
    ...
    let dir = workspace_root.join(format!(
        "temp/improve-log-check-symlink-{}",
        std::process::id()
    ));
    let log_path = dir.join("improve-log.csv");
    let redirected = dir.join("redirected.csv");
    ...
    std::fs::write(
        &redirected,
        concat!(
            "\"timestamp\",\"commit\",\"small_proof_time_s\",",
            "\"large_proof_time_s\",\"summary\"\n",
            "\"2026-06-19T05:55:00-0700\",\"test\",\"1.0\",\"2.0\",\"sentinel\"\n",
        ),
    )
    .expect("redirect target should write");
    symlink(&redirected, &log_path).expect("improve log symlink fixture should be created");
    ...
    assert!(!success, "improve-log check should reject symlinked logs");
    assert!(
        stderr.contains("must not be a symlink"),
        "check-mode symlink rejection should explain the path constraint: stderr={stderr}"
    );
}
