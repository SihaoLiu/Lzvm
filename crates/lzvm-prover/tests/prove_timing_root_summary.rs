...
fn prove_timing_root_summary_reads_sibling_nsys_copy_small_d2h_hints() {
    ...
    std::fs::write(
        ...
        [
            ...
            "1182,626.112,--trace=cuda,nvtx,osrt --sample=cpu --cpuctxsw=process-tree --cudabacktrace=memory:80000",
        ]
        .join("\n"),
    )
    .expect("copy summary fixture should be written");
    ...
    assert_eq!(
        ...
        "--trace=cuda|nvtx|osrt_--sample=cpu_--cpuctxsw=process-tree_--cudabacktrace=memory:80000"
    );
}
...
fn prove_timing_root_summary_reports_trace_report_detail_sample_coverage() {
    ...
    assert!(
        stdout.contains(
            ",10,1.000,10000.000,detail_timing_sampled,100,1500.000,750.000,90.000,270.000,345.000,0.000,30.000,0.000,0.000,150.000,120.000,0.000,15.000,300.000,75.000,row_validation,50.000,profile_row_validation,source_a_value,28.000,98.000,2.000,127.778,0.000,0,0.000,0,0.000,0,0.000,0,0.000,0,0.000,none,0.000,0.000,100.000,20.000,25.000,75.000"
        ),
