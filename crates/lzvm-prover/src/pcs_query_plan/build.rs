...
use crate::witness_commitment::{
    load_witness_commitment_segment_refs_with_shapes, LoadedWitnessCommitmentSegmentRef,
};
...
pub fn build_pcs_query_plan_segment_with_bindings(
    ...
    let witness_segments =
        load_witness_commitment_segment_refs_with_shapes(&schedule.units, witness_segments)
            .map_err(ProvePcsQueryPlanSegmentError::from)?;
    build_pcs_query_plan_segment_with_loaded_binding_refs(
        schedule,
        public_values_hash,
        material_segment,
        &witness_segments,
...
pub(super) fn build_pcs_query_plan_segment_with_loaded_binding_refs<'a>(
    schedule: &ProveSchedule,
    public_values_hash: [u8; 32],
    material_segment: &ProofSegment,
    ...
    let witness_segments = sorted_loaded_witness_commitment_segments(witness_segments)?;
    ...
    for segment in witness_segments.iter().copied() {
        hash_loaded_witness_commitment_segment_for_query_seed(&mut hasher, segment)?;
    }
    ...
    let query_units = collect_loaded_witness_query_units(schedule, &witness_segments)?;
...
pub fn build_pcs_query_plan_segment_from_challenge(
    ...
    let witness_segments =
        load_witness_commitment_segment_refs_with_shapes(&schedule.units, witness_segments)
            .map_err(ProvePcsQueryPlanSegmentError::from)?;
    build_pcs_query_plan_segment_from_loaded_challenge_refs(
        schedule,
        &witness_segments,
        challenge,
        nonce,
...
pub(super) fn build_pcs_query_plan_segment_from_loaded_challenge_refs<'a>(
    schedule: &ProveSchedule,
    witness_segments: &[LoadedWitnessCommitmentSegmentRef<'a>],
    challenge: Ext3,
    nonce: Felt,
...
fn build_pcs_query_plan_segment_from_sorted_challenge<'a>(
    schedule: &ProveSchedule,
...
fn sorted_loaded_witness_commitment_segments<'a>(
    witness_segments: &'a [LoadedWitnessCommitmentSegmentRef<'a>],
) -> Result<Vec<&'a LoadedWitnessCommitmentSegmentRef<'a>>, ProvePcsQueryPlanSegmentError> {
    ...
    out.sort_by_key(|segment| segment.segment.id);
    Ok(out)
}

fn collect_loaded_witness_query_units<'a>(
...
fn hash_loaded_witness_commitment_segment_for_query_seed(
    hasher: &mut Sha256,
    segment: &LoadedWitnessCommitmentSegmentRef<'_>,
) -> Result<(), ProvePcsQueryPlanSegmentError> {
    hasher.update(b"lzvm-witness-commitment-query-seed-v2");
    hasher.update(segment.segment.id.to_le_bytes());
    hasher.update(segment.identity.unit_index.to_le_bytes());
    hasher.update(segment.identity.trace_instance_index.to_le_bytes());
    hasher.update(segment.witness.unit_index.to_le_bytes());
    hasher.update(segment.witness.trace_rows.to_le_bytes());
    hasher.update(segment.witness.trace_columns.to_le_bytes());
    let stage_count = u64::try_from(segment.witness.stages.len())
        .map_err(|_| ProvePcsQueryPlanSegmentError::LengthOverflow)?;
    hasher.update(stage_count.to_le_bytes());
    for stage in &segment.witness.stages {
