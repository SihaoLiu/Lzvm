...
#[test]
fn parallel_seeded_pending_lowerer_emits_trace_order() {
    let _env_lock = GUEST_PC_TRACE_ENV_LOCK
        .lock()
        .expect("guest PC trace env lock should not be poisoned");
    let _mirror_env = TestEnvVarGuard::set("LZVM_GUEST_PC_TRACE_SEED_MIRROR", "1");
    let dir = repo_temp_dir("guest-pc-parallel-seeded-pending-lower");
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).expect("fixture directory should be created");
    let guest_image = dir.join("guest.elf");
    let guest_image_bytes = sample_guest_image_with_words(&[
    ...
    let proof_values = produce_guest_pc_trace_pending_slices(
        16,
        WitnessComputeContext {
            guest_image: Some(&guest_image),
            guest_image_info: Some(&guest_image_info),
    ...
    for segment in pending {
        sender
            .send(GuestPcTracePendingSegmentMessage::Segment(segment))
            .expect("pending segment should send");
    }
    sender
        .send(GuestPcTracePendingSegmentMessage::Complete(Box::new(
            GuestPcTraceStreamResult {
                proof_values: proof_values.clone(),
                timing: GuestPcTraceStreamTiming::default(),
            },
        )))
        .expect("stream completion should send");
    drop(sender);

    let mut emitted = Vec::new();
    let mut timing = GuestPcTraceStreamTiming::default();
    let result = lower_guest_pc_trace_seeded_pending_segments_parallel(
        &layout,
        receiver,
        None,
        &mut timing,
    ...
    .expect("parallel seeded pending segments should lower");

    assert_eq!(result.proof_values, proof_values);
    assert_eq!(emitted.len(), 2);
    assert_eq!(emitted[0].trace_instance_index, 0);
    assert_eq!(emitted[1].trace_instance_index, 1);
    let pc_column = layout.column(1, "pc").expect("pc column").trace_column();
    assert_eq!(
        emitted[0]
            .trace
            .as_ref()
            .expect("first trace should be built")
            .value(0, pc_column),
        Some(Felt::from_canonical(0x8000_0000).expect("canonical first pc"))
    );
    assert_eq!(
        emitted[1]
            .trace
            .as_ref()
            .expect("second trace should be built")
            .value(0, pc_column),
        Some(Felt::from_canonical(0x8000_0008).expect("canonical second pc"))
    );
    assert_eq!(timing.trace_report_count, 3);
}
