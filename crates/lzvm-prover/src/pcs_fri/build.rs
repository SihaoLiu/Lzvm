...
pub fn build_pcs_fri_transcript_commitments_with_timing(
    ...
    for (layer_index, layer) in schedule.fri_layers.iter().enumerate() {
        ...
        let grouped_values =
            group_fri_layer_values(layer_index, current.as_ref(), output_size, folding_factor)?;
    ...
    Ok(PcsFriTranscriptCommitments {
        challenges,
        layer_roots,
        final_polynomial: current.into_owned(),
