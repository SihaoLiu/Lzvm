        ...
        if let Some(fast_path) = report_level_fast_path_parts(row, report, dma_prepare_lookahead)? {
            match fast_path {
                MainReportFastPathParts::FcallResult(instruction, store_index) => {
                    ...
                    let effects =
                        ZiskMainReportEffects::from_fast_path_report(report, Some(store_index));
                ...
                MainReportFastPathParts::NoMemory(instruction, parts) => {
                    ...
                    let effects =
                        ZiskMainReportEffects::from_fast_path_report(report, parts.store_index);
...
fn apply_copy_indirect_register_store_fast_path(
    ...
    let b = single_ordered_memory_access_value(
        output_row,
        effects,
...
#[inline(always)]
fn single_ordered_memory_access_value(
    row: usize,
    effects: ZiskMainReportEffects<'_>,
    kind: GuestMemoryAccessKind,
    address: u64,
