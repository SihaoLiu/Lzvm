...
PIPELINE_ENV_TO_CLEAR = [
    ...
    "LZVM_GUEST_TRACE_SHAPE_TIMING_SAMPLE_STRIDE",
    EXTERNAL_SOURCE_OPENING_BATCH_ENV,
    LEGACY_EXTERNAL_SOURCE_OPENING_BATCH_ENV,
...
def mode_args(args: argparse.Namespace) -> list[str]:
    ...
    if trace_runner_detail_timing_enabled(args):
        result.append("--trace-runner-detail-timing")
    if args.trace_runner_detail_timing_sample_stride is not None:
        result.extend(
            [
                "--trace-runner-detail-timing-sample-stride",
...
def trace_runner_detail_timing_enabled(args: argparse.Namespace) -> bool:
    return bool(
        args.trace_runner_detail_timing
        or args.trace_runner_detail_timing_sample_stride is not None
    )
...
def mode_env_for_args(args: argparse.Namespace, mode: str) -> dict[str, str]:
    ...
    if args.cross_segment_root_window is not None:
        mode_env[CROSS_SEGMENT_ROOT_WINDOW_ENV] = str(args.cross_segment_root_window)
...
def next_command_parts(
    ...
    if args.cross_segment_root_window is not None:
        parts.extend(["--cross-segment-root-window", str(args.cross_segment_root_window)])
    if args.external_source_opening_batch_size is not None:
        parts.extend(
            [
...
def main() -> None:
    ...
    parser.add_argument("--trace-runner-detail-timing", action="store_true")
    parser.add_argument(
        "--trace-runner-detail-timing-sample-stride",
        type=positive_integer,
        default=None,
